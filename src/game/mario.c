...
bool mario_can_bubble(struct MarioState* m) {
    ...
    for (s32 i = 1; i < MAX_PLAYERS; i++) {
        ...
        if (!(gMarioStates[i].visibleToEnemies & MARIO_VISIBLE_TO_OTHERS)) { continue; }
...
void init_single_mario(struct MarioState* m) {
    ...
    m->invincTimer = 0;
    m->visibleToEnemies = (MARIO_VISIBLE_TO_ENEMIES | MARIO_VISIBLE_TO_DETECTORS | MARIO_VISIBLE_TO_COLLISIONS | MARIO_VISIBLE_TO_SYNC | MARIO_VISIBLE_TO_OTHERS | MARIO_VISIBLE_TO_FRIENDLIES);
