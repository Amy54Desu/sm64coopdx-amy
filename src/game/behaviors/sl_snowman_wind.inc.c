...
void bhv_sl_snowman_wind_loop(void) {
    ...
    // Mario has come close, begin dialog.
        ...
        if ((gMarioStates[0].visibleToEnemies & MARIO_VISIBLE_TO_ENEMIES) && cur_obj_update_dialog(&gMarioStates[0], 2, 2, gBehaviorValues.dialogs.SnowmanWindDialog, 0, bhv_sl_snowman_wind_loop_continue_dialog)) {
            o->oSubAction++;
